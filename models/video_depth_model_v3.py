...
class VideoDepthAnything(nn.Module):
    def __init__(
        ...
        #     nn.GELU(),
        #     ZeroConv(1, 1, kernel_size=3, padding=1, bias=True),
        #     TanhScale(out_ch=1, max_scale=1.0, init_log_scale=-4.0)
        # )
        self.final_scale2 = GlobalScaleHead(in_channels=1, out_channels=1, max_log_scale=1.0)

    def forward(self, input_depth):
        B, S, H, W = input_depth.shape
        input_depth = input_depth / 65_535.  # this is depth anything depth
        
        # scale & shift median 맞춤
        # input_depth = match_seq_to_first_median(input_depth, eps=1e-8, scale=True)
        
        if True:
            input_depth_bs1hw = input_depth.unsqueeze(2).flatten(0, 1)
            scale_bs11 = self.final_scale2(input_depth_bs1hw).unflatten(0, (B, S)).squeeze(2)     # (B, S, 1, 1)
        ...
        if self.use_residual:
            ...
            if False:
                output_depth_bs1hw = output_depth.flatten(0, 1).unsqueeze(1)
                scale_bs11 = self.final_scale2(output_depth_bs1hw).squeeze(1).unflatten(0, (B, S))      # (B, S, 1, 1)
                bais_bshw = self.final_res2(output_depth_bs1hw).squeeze(1).unflatten(0, (B, S))
                output_depth = input_depth * scale_bs11 + bais_bshw
            else:
                output_depth = self.final_res2(output_depth.flatten(0, 1).unsqueeze(1))
