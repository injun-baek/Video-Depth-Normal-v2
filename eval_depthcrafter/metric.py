...
def temporal_gradient_matching_error(output, target, valid_mask=None, threshold=0.05):
    """
    Compute Temporal Gradient Matching (TGM) loss
    
    Args:
        pred: [B, S, H, W] predicted tensor
        target: [B, S, H, W] target tensor
        
    Returns:
        tgm loss value
    """
    pred_depth_curr = output[:, :-1]
    pred_depth_next = output[:, 1:]

    gt_depth_curr = target[:, :-1]
    gt_depth_next = target[:, 1:]

    pred_grad = pred_depth_next - pred_depth_curr
    gt_grad = gt_depth_next - gt_depth_curr

    gt_grad_mask = gt_grad < threshold
    mask = gt_grad_mask
    if valid_mask is not None:
        mask = valid_mask[:, :-1] & mask
        
    grad_diff = torch.abs(pred_grad - gt_grad)
    grad_diff[~mask] = 0
    n = mask.sum((-1, -2))
    grad_diff = torch.sum(grad_diff, (-1, -2)) / n
...
def abs_difference(output, target, valid_mask=None):
    ...
    abs_diff = torch.abs(actual_output - actual_target)
    if valid_mask is not None:
